...
exports.getAllBugs = async (req, res) => {
  try {
    ...
    const bugs = await Bug.find({ deviceId, createdBy: req.user._id }).sort({ ScenarioID: 1 });
    res.json(bugs);
  } catch (err) {
...
exports.deleteAllBugs = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Failed to delete all bugs", error: err.message });
  }
};
...
exports.updateBug = async (req, res) => {
  try {
    ...
    if (typeof req.body.ScenarioID === 'string' && req.body.ScenarioID.trim() !== '') {
      ...
      if (clash) {
        return res
          .status(409)
          .json({ message: "Another bug with this ScenarioID already exists for this device & user" });
      }
    }
    ...
    if (err.code === 11000) {
      return res.status(409).json({ message: "Duplicate bug not allowed", error: err.keyValue });
    }
...
exports.getBugSummary = async (req, res) => {
  try {
    ...
    const total = await Bug.countDocuments({ deviceId, createdBy: req.user._id });
    if (total === 0) {
